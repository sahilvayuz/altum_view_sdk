...
//
// Complete AltumView skeleton stream manager.
// Extracted 1-for-1 from AltumSkeletonStreamManager in altum_view_controller.dart.
// NO logic changes — only placement in the correct feature folder.
//
// Responsibilities:
//   1. Check camera online status
//   2. Fetch background image
//   3. Fetch group_id from /info
//   4. Fetch MQTT credentials from /mqttAccount
//   5. Fetch stream token from /cameras/:id/streamtoken
//   6. Connect to MQTT broker via WebSocket
//   7. Publish stream token before subscribing
//   8. Subscribe and parse binary skeleton frames
//   9. Auto-reconnect on disconnect (5 s delay)
//  10. Refresh MQTT creds before expiry (every 5 min check)
// ─────────────────────────────────────────────────────────────────────────────
...
class SkeletonStreamManager {
  ...
  Future<void> start() async {
    ...
    // Re-publish stream token every 45 s (keep stream alive)
    _tokenRefreshTimer = Timer.periodic(
      const Duration(seconds: 45),
          (_) => _publishStreamToken(),
    );

    // Check MQTT creds expiry every 5 min
  ...
  Future<void> _checkCameraStatus() async {
    try {
      ...
      if (camera != null) {
        ...
        if (camera['is_online'] != true) log('⚠️  CAMERA IS OFFLINE');
      }
    } catch (e) {
  ...
  Future<MqttCredentials> _fetchMqttCredentials() async {
    ...
    if (creds.username.isEmpty || creds.password.isEmpty || creds.wssUrl.isEmpty) {
      throw Exception(
  ...
  Future<void> _connectMqtt() async {
    ...
    final port     = Uri.parse(fullUrl).hasPort ? Uri.parse(fullUrl).port : 8084;
    ...
    // Publish token BEFORE subscribing (order matters)
    _publishStreamToken();
  ...
  void _publishStreamToken() {
    ...
    final topic   = 'mobile/$_groupId/camera/$serialNumber/token/mobileStreamToken';
  ...
  //
  // C# reads joint data as flat 36 floats; first 18 = all X, next 18 = all Y.
  // ═══════════════════════════════════════════════════════════════
  ...
  SkeletonFrame _parseBinaryFrame(Uint8List bytes) {
    ...
    if (personCount == 0 || personCount > 20) {
      if (personCount != 0) log('⚠️  personCount=$personCount looks invalid, skipping');
